...
tankpH %>%
  ...
  scale_fill_gradient(low = "#ed872d", high = "#512889")+
  theme_bw()+
...
quadrat %>%
  ...
  ggplot(aes(x = 0, y=density_m2)) + 
    ...
    theme(axis.title = element_text(size = 32), axis.text = element_text(size = 18), 
          axis.text.x = element_blank(), axis.title.x = element_blank())
```
...
mCImodel_avg <- lm(avg_CI ~ pH * temp_treatment, data=musselgrowth_ci_avg) # model
#plot(musselweight)
...
mussel_calc <- mussel_calc %>%
  mutate(calc = ((deltaTA/2)*(1.025)*(volume))/((timediff)))

Mcalc_lm <- lm(calc ~ dry_weight, data=mussel_calc)
summary(Mcalc_lm) # dry weight does not predict calc
...
## testing dissolution probability via log regression
mussel_calc_log <- mussel_calc %>%
  mutate(dissolved = ifelse(umol.cm2.hr <= 0, 1, 0))

mussel_calc_log3 <- mussel_calc_log %>%
  group_by(pH) %>%
  summarize(prop = sum(dissolved))

model <- glm(dissolved ~ pH, 
             data = mussel_calc_log, 
             family = binomial)

summary(model) # nope

## testing calcification probability
mussel_calc_log2 <- mussel_calc %>%
  mutate(calcifying = ifelse(umol.cm2.hr >=0, 1, 0))

model2 <- glm(calcifying ~ pH,
              data = mussel_calc_log2,
              family = binomial)

summary(model2)

## testing dissolution log avg
mussel_calc_log_avg <- mussel_calc_avg %>%
  mutate(avg_dissolved = ifelse(avg_calc <= 0, 1, 0))

model3 <- glm(avg_dissolved ~ pH, 
             data = mussel_calc_log_avg, 
             family = binomial)

summary(model3) # nope

## testing calc log avg
mussel_calc_log_avg2 <- mussel_calc_avg %>%
  mutate(avg_calcifying = ifelse(avg_calc >= 0, 1, 0))

model4 <- glm(avg_calcifying ~ pH, 
             data = mussel_calc_log_avg2, 
             family = binomial)

summary(model4) # nope

# beta regression
mussel_calc_prop <- mussel_calc %>%
  group_by(tank) %>%
  summarise(n_total = n(), # total mussels w calc values per tank
    n_dissolving = sum(umol.cm2.hr < 0, na.rm = TRUE), # number of dissolving mussels
    prop_dissolving = n_dissolving / n_total, # proportion of dissolving mussels
    .groups = "drop",
    across(pH)) %>%
  mutate(prop_dissolving_adj = ((prop_dissolving * (n_total - 1)) + 0.5) / n_total)

# using 0.000001 and 0.999999
mussel_calc_prop <- mussel_calc %>%
  group_by(tank) %>%
  summarise(n_total = n(), # total mussels w calc values per tank
    n_dissolving = sum(umol.cm2.hr < 0, na.rm = TRUE), # number of dissolving mussels
    ...
    across(pH)) %>%
  mutate(
    prop_dissolving_adj = case_when(
      prop_dissolving == 0 ~ 0.01,
      prop_dissolving == 1 ~ 0.99,
      TRUE ~ prop_dissolving
    )
  )

betamodel <- betareg(prop_dissolving_adj ~ pH, data = mussel_calc_prop)
summary(betamodel)
```
...
```{r}
# determine metabolic scaling of respiration rate 
musrespoweight <- lm(log(Respiration) ~ log(AFDW) + pH + temp_treatment, data=musselrespo)
musrespoweight_all <- lm(log(Respiration) ~ log(AFDW) * pH * temp_treatment, data=musselrespo)

# test if interactions
anova(musrespoweight,musrespoweight_all) # no interactions between pH x AFDW or temp x AFDW
anova(musrespoweight_all) # no interactions between pH x AFDW or temp x AFDW

anova(musrespoweight) # temp and AFDW significant
summary(musrespoweight) # slope is only ~0.6, should be closer to 0.75 per kleiber's law
plot(musrespoweight)
...
nucrespoweight <- lm(log(Respiration) ~ log(AFDW) + pH + temp_treatment, data = nucellarespo) # does AFDW predict respiration rate?
nucrespoweight_all <- lm(log(Respiration) ~ log(AFDW) * pH * temp_treatment, data = nucellarespo) 
anova(nucrespoweight,nucrespoweight_all)
summary(nucrespoweight)

anova(nucrespoweight) # weight not significant

summary(nucellarespo)
...
#shellgrowthplot + Nrespoplot + Ncalcplot + plot_layout(guides = "collect", axes = "collect_x") &   theme(axis.title = element_text(size = 18), axis.text = element_text(size=16))
