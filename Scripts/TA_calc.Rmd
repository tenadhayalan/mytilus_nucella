...
```{r}
respo <- read_csv(here("Data","PR_2024","Metadata_shells.csv"))
calc <- read_csv(here("Data", "TA", "Calcification_shells.csv"))
...
full.calc <- calc %>%
  rename("Species" = "sample_type") %>%
  left_join(time.data, by = c("tank", "Species"))
...
clean.full.calc$deltaTA<- (clean.full.calc$TA_initial - clean.full.calc$TA)
...
clean.full.calc$umol.cm2.hr <-((clean.full.calc$deltaTA/2)*(1.025)*(clean.full.calc$volume))/((clean.full.calc$timediff))


#write_csv(clean.full.calc, here("Data","TA","shell_NEC.csv"))
```
